...
var timer: Timer
var daemon_url := "http://127.0.0.1:32123"

func _enter_tree() -> void:
	toolbar_button = Button.new()
	toolbar_button.text = "🔄 Sync ChatGPT"
	toolbar_button.tooltip_text = "Tarik kode terbaru dari ChatGPT / GitHub ke laptop"
	...
	client.request_completed.connect(func(result, code, headers, body):
		if code == 200:
			...
			if json and json.has("behind") and json["behind"] > 0:
				toolbar_button.text = "🟡 ChatGPT Update! (" + str(json["behind"]) + ")"
...
func _on_request_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedStringArray) -> void:
	...
	if response_code == 200:
		print("[ChatGPT Sync] Berhasil menarik pembaruan dari ChatGPT/GitHub!")
		get_editor_interface().get_resource_filesystem().scan()
	else:
		print("[ChatGPT Sync] Gagal terhubung ke daemon localhost:32123")